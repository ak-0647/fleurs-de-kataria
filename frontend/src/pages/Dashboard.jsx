...
export default function Dashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', paddingTop: '120px', display: 'flex', flexDirection: 'column', alignItems: 'center', background: 'var(--bg-deep)', color: '#FFF', position: 'relative', overflowX: 'hidden' }}>
      
      {/* Ambient Animated Glowing Background Orbs */}
      <div ref={orb1Ref} style={{ position: 'absolute', top: '15%', left: '10%', width: '400px', height: '400px', background: 'radial-gradient(circle, rgba(230,0,69,0.2) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(60px)', zIndex: 0, pointerEvents: 'none', borderRadius: '50%' }}></div>
      <div ref={orb2Ref} style={{ position: 'absolute', top: '50%', right: '5%', width: '500px', height: '500px', background: 'radial-gradient(circle, rgba(156,19,85,0.2) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(70px)', zIndex: 0, pointerEvents: 'none', borderRadius: '50%' }}></div>
      <div ref={orb3Ref} style={{ position: 'absolute', bottom: '10%', left: '30%', width: '600px', height: '600px', background: 'radial-gradient(circle, rgba(251,226,159,0.1) 0%, rgba(0,0,0,0) 70%)', filter: 'blur(80px)', zIndex: 0, pointerEvents: 'none', borderRadius: '50%' }}></div>


      {/* Welcome Card */}
      <div ref={cardRef} style={{ background: 'rgba(255,255,255,0.03)', padding: '4rem', borderRadius: '16px', border: '1px solid rgba(230,0,69,0.2)', textAlign: 'center', maxWidth: '650px', width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.6)', marginBottom: '5rem', position: 'relative', zIndex: 10, backdropFilter: 'blur(16px)' }}>
        <h1 style={{ fontFamily: 'Cinzel, serif', color: '#FBE29F', marginBottom: '1rem', fontSize: '3rem' }}>Welcome, {user.name}</h1>
        <p style={{ color: '#A19BAA', marginBottom: '2rem', fontSize: '1.2rem', letterSpacing: '4px', textTransform: 'uppercase' }}>Authentication Level: {user.role}</p>
        
        <p style={{ marginBottom: '3rem', color: '#FFF', fontSize: '1rem', fontWeight: 300 }}>
      ...
      <div ref={aboutRef} style={{ maxWidth: '900px', width: '90%', textAlign: 'center', marginBottom: '5rem', padding: '4rem 3rem', background: 'rgba(15, 5, 10, 0.4)', backdropFilter: 'blur(24px)', border: '1px solid rgba(230,0,69,0.15)', borderRadius: '24px', position: 'relative', zIndex: 10, boxShadow: '0 30px 60px rgba(0,0,0,0.6)' }}>
        <h2 style={{ fontFamily: 'Cinzel, serif', color: '#E60045', fontSize: '2.5rem', marginBottom: '2rem', textShadow: '0 0 20px rgba(230,0,69,0.5)' }}>The Art of Fleurs de Kataria</h2>
        <div style={{ color: '#D5D0DB', fontSize: '1.15rem', lineHeight: '1.9', fontWeight: 300, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
      ...
      <footer ref={footerRef} style={{ width: '100%', position: 'relative', zIndex: 10, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', borderTop: '1px solid rgba(255,255,255,0.05)', padding: '4rem 2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2rem', marginTop: 'auto' }}>
        <h3 style={{ fontFamily: 'Cinzel, serif', color: '#FBE29F', fontSize: '1.8rem', letterSpacing: '2px', margin: 0 }}>Fleurs de Kataria</h3>
        
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '3rem', color: '#A19BAA' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <FaMapMarkerAlt color="#E60045" size={20} />
            <span>YNR, Haryana</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <FaEnvelope color="#E60045" size={20} />
            <a href="mailto:akshitasharma1205@gmail.com" style={{ color: '#A19BAA', textDecoration: 'none', transition: 'color 0.3s' }} onMouseOver={e => e.target.style.color='#FFF'} onMouseOut={e => e.target.style.color='#A19BAA'}>akshitasharma1205@gmail.com</a>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.8rem' }}>
            <FaPhoneAlt color="#E60045" size={20} />
            <a href="tel:9416427080" style={{ color: '#A19BAA', textDecoration: 'none', transition: 'color 0.3s' }} onMouseOver={e => e.target.style.color='#FFF'} onMouseOut={e => e.target.style.color='#A19BAA'}>+91 94164 27080</a>
          </div>
        </div>
