      ...
      {STATUS_STEPS.map((step, idx) => {
        ...
        return (
          <div key={step} style={{ zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.8rem' }}>
            ...
            <span style={{ fontSize: '0.7rem', color: isActive ? 'var(--accent)' : 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: isCurrent ? 'bold' : 'normal' }}>{step}</span>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div style={{ paddingTop: '100px', minHeight: '100vh', background: 'var(--bg-deep)', color: 'var(--text-main)' }}>
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem' }}>
        <h1 style={{ fontFamily: 'Cinzel, serif', fontSize: '3.5rem', color: 'var(--accent)', marginBottom: '4rem', textAlign: 'center' }}>My Orders & Masterpieces</h1>
        
        {orders.length === 0 && customRequests.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '10rem', background: 'var(--glass)', borderRadius: '24px', border: '1px dashed rgba(251,226,159,0.1)' }}>
            <p style={{ color: 'var(--text-muted)', fontSize: '1.2rem', fontFamily: 'Cinzel, serif', letterSpacing: '2px' }}>Your floral story hasn't begun yet.</p>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
            {orders.map(order => (
              <div key={order.id} style={{ background: 'var(--glass)', border: '1px solid rgba(230,0,69,0.15)', borderRadius: '24px', overflow: 'hidden', backdropFilter: 'blur(10px)' }}>
                <div 
                ...
                >
                  <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
                    <div style={{ textAlign: 'center', minWidth: '80px' }}>
                        <p style={{ margin: 0, fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '1px' }}>Order</p>
                        <p style={{ margin: 0, fontSize: '1.5rem', color: 'var(--accent)', fontFamily: 'Cinzel, serif' }}>#{order.id}</p>
                    </div>
                    <div style={{ borderLeft: '1px solid rgba(255,255,255,0.1)', paddingLeft: '2rem' }}>
                      <h3 style={{ margin: 0, fontFamily: 'Cinzel, serif', fontSize: '1.2rem', color: 'var(--text-main)' }}>
                        {new Date(order.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' })}
                      </h3>
                      <p style={{ margin: '0.3rem 0 0', color: 'var(--text-muted)', fontSize: '0.8rem' }}>{order.items.length} unique treasures</p>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right', display: 'flex', alignItems: 'center', gap: '3rem' }}>
                    <div>
                      <span style={{ color: 'var(--accent)', fontSize: '1.6rem', fontWeight: 'lighter', fontFamily: 'Cinzel, serif' }}>₹{order.total_amount}</span>
                      <p style={{ margin: 0, color: 'var(--primary)', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '2px', marginTop: '4px' }}>{order.status}</p>
                    </div>
                ...
                {expandedOrder === order.id && (
                  <div style={{ padding: '0 2rem 2.5rem', borderTop: '1px solid rgba(255,255,255,0.03)' }}>
                    ...
                    <div style={{ marginTop: '3rem' }}>
                      <h4 style={{ color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '3px', fontSize: '0.75rem', marginBottom: '2rem', textAlign: 'center' }}>Composition</h4>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1.5rem' }}>
                        {order.items.map(item => (
                          <div key={item.id} style={{ display: 'flex', gap: '1.5rem', background: 'var(--glass)', padding: '1.2rem', borderRadius: '16px', border: '1px solid var(--glass)' }}>
                            ...
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                              <p style={{ margin: 0, fontWeight: 'bold', color: 'var(--text-main)' }}>{item.name}</p>
                              <p style={{ margin: '0.2rem 0', color: 'var(--text-muted)', fontSize: '0.8rem' }}>Quantity: {item.quantity}</p>
                              <p style={{ margin: 0, color: 'var(--accent)', fontSize: '0.9rem' }}>₹{item.price * item.quantity}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ marginTop: '2.5rem', padding: '1.5rem', background: 'rgba(230,0,69,0.03)', borderRadius: '16px', border: '1px solid rgba(230,0,69,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <h4 style={{ color: 'var(--primary)', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '0.5rem' }}>Delivery Destination</h4>
            ...
            {customRequests.length > 0 && (
              <div style={{ marginTop: '5rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '2rem', marginBottom: '3rem' }}>
                    ...
                    <h2 style={{ fontFamily: 'Cinzel, serif', color: 'var(--accent)', fontSize: '2rem', margin: 0 }}>Bespoke Masterpieces</h2>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(450px, 1fr))', gap: '2rem' }}>
                  {customRequests.map(req => (
                    <div key={req.id} style={{ background: 'var(--glass)', border: '1px solid var(--glass-border)', padding: '2rem', borderRadius: '24px', display: 'flex', flexDirection: 'column', gap: '1.5rem', position: 'relative', overflow: 'hidden' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <div>
                          <span style={{ color: 'var(--accent)', fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '2px', border: '1px solid rgba(251,226,159,0.3)', padding: '2px 8px', borderRadius: '10px' }}>Inquiry #{req.id}</span>
                          <h3 style={{ margin: '1rem 0 0.5rem', fontFamily: 'Cinzel, serif', fontSize: '1.4rem' }}>{req.flower_selection}</h3>
                          <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.8rem' }}>Requested on {new Date(req.created_at).toLocaleDateString()}</p>
                        </div>
                        <div style={{ textAlign: 'right' }}>
                           <p style={{ margin: 0, color: 'var(--accent)', fontSize: '1.2rem', fontFamily: 'Cinzel, serif' }}>₹{req.budget || '---'}</p>
                           <p style={{ margin: '0.2rem 0 0', color: 'var(--text-muted)', fontSize: '0.7rem', textTransform: 'uppercase' }}>Estimated Budget</p>
                        </div>
                      </div>

                      <div style={{ background: 'rgba(0,0,0,0.2)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--glass)' }}>
                         <BespokeTracker currentStatus={req.status} />
                         <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem' }}>
                            <p style={{ margin: 0, color: 'var(--accent)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px' }}>Current State</p>
                            <p style={{ margin: 0, color: 'var(--text-main)', fontSize: '0.85rem', fontWeight: 'bold' }}>{req.status}</p>
                         </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                        ...
                        <div style={{ flex: 1 }}>
                           <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: '0.8rem', fontStyle: 'italic' }}>"{req.note || "No custom note provided."}"</p>
                        </div>
                      </div>
                    </div>
                  ))}
