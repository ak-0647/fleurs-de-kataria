    ...
    <div style={{ paddingTop: '100px', minHeight: '100vh', background: 'var(--bg-deep)', color: '#FFF' }}>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
          <h1 style={{ fontFamily: 'Cinzel, serif', color: '#FBE29F', fontSize: '3rem', margin: 0 }}>The Atelier Guild</h1>
          <p style={{ color: '#A19BAA', letterSpacing: '4px', textTransform: 'uppercase', margin: 0 }}>Administrator Portal</p>
        </div>
        
        <div style={{ display: 'flex', gap: '2rem', marginBottom: '3rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
          <button onClick={() => setActiveTab('orders')} style={{ padding: '1rem 2rem', background: 'transparent', border: 'none', color: activeTab === 'orders' ? '#E60045' : '#A19BAA', cursor: 'pointer', fontSize: '1rem', fontFamily: 'Cinzel, serif', textTransform: 'uppercase', letterSpacing: '2px', borderBottom: activeTab === 'orders' ? '2px solid #E60045' : 'none', transition: '0.3s' }}>Orders</button>
          <button onClick={() => setActiveTab('requests')} style={{ padding: '1rem 2rem', background: 'transparent', border: 'none', color: activeTab === 'requests' ? '#E60045' : '#A19BAA', cursor: 'pointer', fontSize: '1rem', fontFamily: 'Cinzel, serif', textTransform: 'uppercase', letterSpacing: '2px', borderBottom: activeTab === 'requests' ? '2px solid #E60045' : 'none', transition: '0.3s' }}>Bespoke Requests</button>
          <button onClick={() => setActiveTab('flowers')} style={{ padding: '1rem 2rem', background: 'transparent', border: 'none', color: activeTab === 'flowers' ? '#E60045' : '#A19BAA', cursor: 'pointer', fontSize: '1rem', fontFamily: 'Cinzel, serif', textTransform: 'uppercase', letterSpacing: '2px', borderBottom: activeTab === 'flowers' ? '2px solid #E60045' : 'none', transition: '0.3s' }}>Catalog</button>
        </div>
          ...
          <div>
            {activeTab === 'orders' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100%, 1fr))', gap: '2rem' }}>
                {orders.length === 0 ? <p style={{color: '#A19BAA', textAlign: 'center', padding: '5rem'}}>No transactions recorded.</p> : orders.map(order => (
                  <div key={order.id} style={{ background: 'rgba(255,255,255,0.02)', padding: '2rem', borderRadius: '16px', border: '1px solid rgba(230,0,69,0.15)', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '2rem' }}>
                    <div style={{ flex: '1 1 400px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
                        ...
                        <Link to={`/admin/orders/${order.id}`} style={{ color: '#E60045', fontSize: '0.8rem', textTransform: 'uppercase', textDecoration: 'none' }}>Detailed View</Link>
                        <span style={{ background: 'rgba(5,3,8,0.8)', padding: '0.3rem 1rem', borderRadius: '20px', border: '1px solid #E60045', color: '#E60045', fontSize: '0.7rem', textTransform: 'uppercase' }}>{order.status}</span>
                      </div>
                      <p style={{ margin: '0 0 1.5rem 0', color: '#A19BAA', fontSize: '0.85rem' }}>{new Date(order.created_at).toLocaleString('en-IN', { dateStyle: 'long', timeStyle: 'short' })}</p>
                      
                      <div style={{ marginBottom: '1.5rem' }}>
                        <p style={{ margin: '0 0 0.5rem 0', color: '#FFF' }}><strong>Recipient:</strong> {order.full_name}</p>
                        <p style={{ margin: '0 0 0.5rem 0', color: '#FFF' }}><strong>Contact:</strong> {order.email} | {order.phone}</p>
                        <p style={{ margin: '0 0 0.5rem 0', color: '#A19BAA', fontSize: '0.9rem' }}><strong>Ship to:</strong> {order.delivery_address}</p>
                      </div>

                      <h4 style={{ fontFamily: 'Cinzel, serif', color: '#E60045', marginBottom: '1rem', fontSize: '0.9rem', textTransform: 'uppercase' }}>Composition (Total: ₹{order.total_amount}):</h4>
                        ...
                        {order.items?.map(item => (
                          <div key={item.id} style={{ background: 'rgba(255,255,255,0.03)', padding: '0.8rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            ...
                            <div>
                                ...
                                <p style={{ margin: 0, fontSize: '0.75rem', color: '#A19BAA' }}>{item.quantity} units @ ₹{item.price}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div style={{ flex: '0 0 320px', background: 'rgba(0,0,0,0.3)', padding: '2rem', borderRadius: '12px', border: '1px solid rgba(251,226,159,0.1)' }}>
                      <h4 style={{ fontFamily: 'Cinzel, serif', color: '#FBE29F', fontSize: '1rem', marginBottom: '1.5rem', textAlign: 'center' }}>Fulfillment</h4>
                      <div style={{ marginBottom: '1.2rem' }}>
                        <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.7rem', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Current Status</label>
                      ...
                      <div style={{ marginBottom: '1.2rem' }}>
                        <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.7rem', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Curated By</label>
                      ...
                      <div style={{ marginBottom: '1.5rem' }}>
                        <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.7rem', textTransform: 'uppercase', marginBottom: '0.5rem' }}>Delivery Window</label>
            ...
            {activeTab === 'requests' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                {requests.length === 0 ? <p style={{color: '#A19BAA', textAlign: 'center', padding: '5rem'}}>No bespoke inquiries.</p> : requests.map(req => (
                  <div key={req.id} style={{ background: 'rgba(255,255,255,0.02)', padding: '2rem', borderRadius: '16px', border: '1px solid rgba(251,226,159,0.15)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '3rem' }}>
                      <div style={{ flex: '1 1 500px' }}>
                        <h3 style={{ fontFamily: 'Cinzel, serif', color: '#FBE29F', margin: '0 0 1rem 0', fontSize: '1.8rem' }}>Bespoke Inquiry #{req.id}</h3>
                        <div style={{ background: 'rgba(230,0,69,0.05)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(230,0,69,0.1)', marginBottom: '1.5rem' }}>
                            <p style={{ margin: '0 0 0.8rem 0', color: '#E60045', textTransform: 'uppercase', fontSize: '0.75rem', letterSpacing: '2px' }}>Customer Brief</p>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <div><p style={{ color: '#A19BAA', margin: 0 }}>FLORAL CHOICE</p><p style={{ margin: '0.2rem 0 0 0' }}>{req.flower_selection}</p></div>
                                <div><p style={{ color: '#A19BAA', margin: 0 }}>WRAPPING</p><p style={{ margin: '0.2rem 0 0 0' }}>{req.wrapping_style}</p></div>
                                <div><p style={{ color: '#A19BAA', margin: 0 }}>RIBBONS</p><p style={{ margin: '0.2rem 0 0 0' }}>{req.ribbon_color || 'Standard'}</p></div>
                                <div><p style={{ color: '#A19BAA', margin: 0 }}>BUDGET</p><p style={{ margin: '0.2rem 0 0 0', color: '#FBE29F' }}>₹{req.budget || 'Open'}</p></div>
                            </div>
                        </div>
                        <p style={{ background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '8px', borderLeft: '4px solid var(--accent)', color: '#D5D0DB', lineClamp: '3', overflow: 'hidden' }}>
                      ...
                      {req.image_path && (
                        <div style={{ flex: '0 0 250px' }}>
                          <p style={{ margin: '0 0 1rem 0', color: '#A19BAA', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '1px', textAlign: 'center' }}>Inspiration Reference</p>
                      ...
                      <div style={{ flex: '0 0 250px', background: 'rgba(0,0,0,0.3)', padding: '1.5rem', borderRadius: '12px', border: '1px solid rgba(230,0,69,0.1)' }}>
                        <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.75rem', textTransform: 'uppercase', marginBottom: '1rem', textAlign: 'center' }}>Set Inquiry Status</label>
                        ...
                        <p style={{ fontSize: '0.7rem', color: '#A19BAA', textAlign: 'center' }}>Setting to 'Accepted' will notify the client via their registered portal.</p>
                      </div>
                    </div>
                  </div>
                ))}
                  ...
                  {flowers.map(f => (
                    <div key={f.id} style={{ background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                      ...
                      <div style={{ padding: '1.5rem' }}>
                        <h3 style={{ fontFamily: 'Cinzel, serif', color: '#FFF', margin: '0 0 0.5rem 0' }}>{f.name}</h3>
                        <p style={{ color: '#FBE29F', fontFamily: 'Cinzel, serif', marginBottom: '1rem' }}>₹{f.price}</p>
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
                           <span style={{ fontSize: '0.7rem', background: 'rgba(230,0,69,0.1)', color: '#E60045', padding: '0.2rem 0.6rem', borderRadius: '4px' }}>{f.category}</span>
                        ...
                        <div style={{ display: 'flex', gap: '1rem' }}>
                          <button onClick={() => openEditModal(f)} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#FFF', padding: '0.6rem', borderRadius: '4px', cursor: 'pointer' }}>Edit</button>
                          <button onClick={() => deleteFlower(f.id)} style={{ flex: 1, background: 'rgba(230,0,69,0.1)', border: '1px solid rgba(230,0,69,0.2)', color: '#E60045', padding: '0.6rem', borderRadius: '4px', cursor: 'pointer' }}>Delete</button>
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div style={{ background: '#0D050A', border: '1px solid #FBE29F', padding: '3rem', borderRadius: '24px', width: '600px', maxWidth: '90%' }}>
              ...
              <form onSubmit={handleFlowerSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                <div style={{ gridColumn: 'span 2' }}>
                  <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Full Name</label>
                  <input required className="filter-input" value={flowerForm.name} onChange={e => setFlowerForm({...flowerForm, name: e.target.value})} />
                </div>
                <div style={{ gridColumn: 'span 2' }}>
                  <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Description</label>
                ...
                <div>
                  <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Price (₹)</label>
                ...
                <div>
                  <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Category</label>
                ...
                <div>
                  <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Color</label>
                ...
                <div>
                  <label style={{ display: 'block', color: '#A19BAA', fontSize: '0.8rem', marginBottom: '0.5rem' }}>Occasion</label>
